...
public class EstudiantesP {
    ...
    private static void buscarEstudiantePorNombre() {
        ...
        if (indice == -1) {
            ...
            JOptionPane.showMessageDialog(null, "Se encontró a: " + nombres[indice] + " - " + edades[indice]);
        }
    }

    private static void buscarEstudiantesPorEdad() {
        int edad = Integer.parseInt(JOptionPane.showInputDialog(null, "Ingrese la edad buscada:"));
        String estudiantes = "estudiantes con esa edad " + edad + ":\n\n";
        for (int i = 0; i < numEstudiantes; i++) {
            if (edades[i] == edad) {
                estudiantes += nombres[i] + " - " + edades[i] + "\n";
            }
